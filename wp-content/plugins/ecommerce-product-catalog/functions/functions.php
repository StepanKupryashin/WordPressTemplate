...
function get_product_catalog_page_title() {
	if ( is_ic_taxonomy_page() ) {
		...
		$search_keyword = apply_filters( 'ic_search_keayword', strval( $_GET[ 's' ] ) );
		if ( !empty( $search_keyword ) ) {
			$title = __( 'Search Results for:', 'ecommerce-product-catalog' ) . ' <span class="ic-search-keyword">' . $search_keyword . '</span>';
		} else {
			$title = '';
		}
...
function ic_pre_get_products( $query ) {
	if ( ((!is_admin() && $query->is_main_query()) || is_ic_ajax()) && !isset( $_GET[ 'order' ] ) && ((!empty( $query->query[ 'post_type' ] ) && ic_string_contains( $query->query[ 'post_type' ], 'al_product' )) || is_ic_product_listing( $query ) || is_ic_taxonomy_page( $query ) || is_ic_product_search( $query ) || is_home_archive( $query )) ) {
		...
		if ( (!empty( $query->query[ 'post_type' ] ) && ic_string_contains( $query->query[ 'post_type' ], 'al_product' ) && empty( $query->query[ 'name' ] )) || (!empty( $query->query ) && is_array( $query->query ) && (ic_string_contains( implode( '::', array_keys( $query->query ) ), 'al_product-cat' ) || !empty( $query->query[ 'al_product-cat' ] ))) ) {
...
function ic_handle_back_post_thumbnail() {
	...
	remove_filter( 'has_post_thumbnail', 'ic_override_product_post_thumbnail', 10, 2 );
}
...
function ic_override_product_post_thumbnail( $metadata, $object_id, $meta_key = null ) {
	...
	if ( ((isset( $meta_key ) && $meta_key == '_thumbnail_id') || !isset( $meta_key )) && is_ic_product( $object_id ) ) {
		$metadata = 0;
	}
	return $metadata;
}
...
function ic_visible_product_status() {
	return apply_filters( 'ic_visible_product_status', array( 'publish' ) );
}
