...
function ic_admin_add_import_tab() {
	?>
	<a id="csv-settings" class="nav-tab<?php echo $class ?>" href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=product-settings&submenu=csv' ) ?>"><?php _e( 'Import / Export', 'ecommerce-product-catalog' ); ?></a>
	<?php
}
	...
	<?php if ( $submenu == 'csv' ) { ?>
		<div class="setting-content submenu csv-tab">
			...
			if ( $export == 1 ) {
				...
				?>
				<a style="display: block; margin-top: 20px;" href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=product-settings&submenu=csv&export_csv=1' ) ?>"><button class="button simple-export-button" ><?php _e( "Export all items to CSV file", 'ecommerce-product-catalog' ) ?></button></a>
