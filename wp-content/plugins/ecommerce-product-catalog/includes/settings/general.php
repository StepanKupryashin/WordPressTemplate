...
function general_menu() {
	if ( current_user_can( 'manage_product_settings' ) ) {
		?>
		<a id="general-settings" class="nav-tab" href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=product-settings' ) ?>"><?php _e( 'General Settings', 'ecommerce-product-catalog' ); ?></a>
		<?php
	}
}
	...
	<div class="overall-product-settings settings-wrapper" style="clear:both;">
		...
		<?php if ( $submenu == 'general-settings' OR $submenu == '' ) { ?>
			<div class="setting-content submenu">
				...
				<a style="display: inline-block;vertical-align: middle;margin-left: 10px;" class="button-secondary" href="<?php echo esc_url( admin_url( 'edit.php?post_type=al_product&page=implecode_welcome' ) ) ?>"><?php _e( 'Configuration Wizard', 'ecommerce-product-catalog' ) ?></a>
				...
				<form method="post" action="options.php">
					...
					if ( !is_advanced_mode_forced() || ic_is_woo_template_available() || is_ic_shortcode_integration() ) {
						...
						<div class="ic-important-settings">
							...
							<span><span style="vertical-align: middle;" title="<?php echo $tip ?>" class="dashicons dashicons-editor-help ic_tip"></span><?php _e( 'Main Catalog Page', 'ecommerce-product-catalog' ); ?>: </span>
							<?php
							...
							if ( ic_has_listing_shortcode() ) {
								if ( !is_integraton_file_active() ) {
									...
									<table>
										...
										implecode_settings_radio( __( 'Layout mode', 'ecommerce-product-catalog' ), 'ic-fake-empty-feature', 'simple', array( 'advanced' => $descriptions[ 'advanced' ], 'theme' => $descriptions[ 'theme' ], 'simple' => $descriptions[ 'simple' ] ), 1, __( 'Simple mode is recommended. Choose Advanced Mode if you want more control over the layout or theme mode if your theme should control the layout. ', 'ecommerce-product-catalog' ), '<br>', "integration-mode-selection" );
										?>
									</table>
									<?php
								}
...
//echo sample_product_button( 'p', __( 'Remove the Shortcode and Start Visual Wizard', 'ecommerce-product-catalog' ), 'button' );
					...
					if ( file_exists( AL_BASE_PATH . '/modules/cart/index.php' ) ) {
						...
						<table><?php
							implecode_settings_radio( __( 'Catalog Mode', 'ecommerce-product-catalog' ), 'archive_multiple_settings[catalog_mode]', $archive_multiple_settings[ 'catalog_mode' ], array( 'store' => __( 'Web Store', 'ecommerce-product-catalog' ), 'inquiry' => __( 'Inquiry Catalog', 'ecommerce-product-catalog' ), 'affiliate' => __( 'Affiliate Catalog', 'ecommerce-product-catalog' ), 'simple' => __( 'Simple Catalog', 'ecommerce-product-catalog' ) ), 1, __( 'Choose your usage scenario.', 'ecommerce-product-catalog' ) );
							?>
						</table>
						<?php
					}
					...
					<table>
						...
						implecode_settings_radio( __( 'Main listing shows', 'ecommerce-product-catalog' ), 'archive_multiple_settings[product_listing_cats]', $archive_multiple_settings[ 'product_listing_cats' ], array( 'off' => $uc_item_name, 'on' => $uc_item_name . ' & ' . __( 'Main Categories', 'ecommerce-product-catalog' ), 'cats_only' => sprintf( __( 'Main Categories & Uncategorized %s', 'ecommerce-product-catalog' ), $uc_item_name ) ) );
					...
					<div class="advanced_mode_settings_inline">
						...
						<table>
							<?php if ( is_ic_permalink_product_catalog() ) { ?>
								<tr>
									<td>
										<span title="<?php echo sprintf( __( 'By default, the category parent slug cannot be the same as for products. If you want them to be the same, please look for %s extension in the catalog extensions menu.', 'ecommerce-product-catalog' ), 'Smarter Product URLs' ) ?>" class="dashicons dashicons-editor-help ic_tip"></span>
