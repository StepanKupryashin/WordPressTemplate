...
class ic_register_product {
	...
	function __construct() {
		...
		add_filter( 'get_post_metadata', array( $this, 'hidden_product_data' ), 10, 3 );
	...
	function frontend_scripts() {
		...
		wp_register_script( 'ic_magnifier', AL_PLUGIN_BASE_PATH . 'js/magnifier/magnifier.js' . ic_filemtime( AL_BASE_PATH . '/js/magnifier/magnifier.js' ), false, true );
		wp_register_script( 'colorbox', AL_PLUGIN_BASE_PATH . 'js/colorbox/jquery.colorbox-min.js', array( 'jquery' ), false, true );
...
//$product_listing_t	 = $slug;
	...
	function implecode_save_products_meta( $post_id, $post, $post_prev = null ) {
		$hook_existed	 = remove_filter( 'get_post_metadata', array( $this, 'hidden_product_data' ), 10, 3 );
		...
		if ( $post_type_now == 'al_product' ) {
			...
			do_action( 'product_edit_save', $post, $product_meta, $post_prev );
		}
		if ( $hook_existed ) {
			add_filter( 'get_post_metadata', array( $this, 'hidden_product_data' ), 10, 3 );
		}
	...
	function save_meta( $post, $product_meta, $value = null ) {
		...
		if ( in_array( $post->post_status, ic_visible_product_status() ) ) {
			...
			if ( !isset( $_POST[ 'action' ] ) || (isset( $_POST[ 'action' ] ) && $_POST[ 'action' ] != 'editpost') ) {
				$current_hiden = get_post_meta( $post->ID, '_ic_hidden_product_data', true );
				if ( !empty( $current_hiden ) ) {
					foreach ( $current_hiden as $hidden_key => $hidden_value ) {
			...
			update_post_meta( $post->ID, '_ic_hidden_product_data', $product_meta );
	...
	function save_single_meta( $product_id, $key, $value, $check_status = false ) {
		...
		if ( is_array( $current_post_keys ) && in_array( $key, $current_post_keys ) ) {
			$current_value = get_post_meta( $product_id, $key, true );
		}
		if ( (!empty( $value ) || is_numeric( $value ) ) && !isset( $current_value ) ) {
			add_post_meta( $product_id, $key, $value, true );
			$this->save_filterable_meta( $product_id, $key, $value );
		} else if ( (!empty( $value ) || is_numeric( $value )) && $value != $current_value ) {
			update_post_meta( $product_id, $key, $value );
			$this->save_filterable_meta( $product_id, $key, $value );
		} else if ( empty( $value ) && !is_numeric( $value ) && isset( $current_value ) ) {
	...
	function hidden_product_data( $metadata, $object_id, $meta_key = null ) {
		if ( !empty( $object_id ) ) {
		...
		if ( empty( $metadata ) && !empty( $meta_key ) && $meta_key !== '_ic_hidden_product_data' ) {
			$hidden_data = get_post_meta( $object_id, '_ic_hidden_product_data', true );
			if ( !empty( $hidden_data[ $meta_key ] ) ) {
