	...
	if ( $page != '' && !is_ic_shortcode_integration() ) {
		if ( get_integration_type() != 'simple' ) {
			if ( ic_has_page_catalog_shortcode( $page ) ) {
				$page->post_content = str_replace( array( '<!-- wp:ic-epc/show-catalog /-->', '[show_product_catalog]' ), '', $page->post_content );
			}
...
function ic_home_listing_query_args() {
	...
	$args = apply_filters( 'home_product_listing_query', array( 'post_status' => ic_visible_product_status(), 'post_type' => 'al_product', 'posts_per_page' => $multiple_settings[ 'archive_products_limit' ], 'paged' => $paged ) );
	return $args;
}
