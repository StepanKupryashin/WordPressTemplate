...
function al_product_attributes( $post, $args = null ) {
	...
	for ( $i = 1; $i <= $attributes_number; $i++ ) {
		...
		?>
		<tr>
			<td class="attributes-label-column"><input <?php echo $label_autocomplete ?> class="ic_autocomplete attribute-label" type="text" data-base_name="<?php echo $names[ 'label' ] ?>" name="<?php echo $names[ 'label' ] . $i ?>" value="<?php echo esc_html( $attributes_label_option_field ) ?>"/></td>
			...
			<td class="unit-column"><input class="attribute-unit admin-number-field" type="text" data-base_name="<?php echo $names[ 'unit' ] ?>" name="<?php echo $names[ 'unit' ] . $i ?>" value="<?php echo esc_html( $attributes_unit_option_field ) ?>"/></td>
			<td class="dragger"></td>
		</tr>
	<?php } ?>
	</tbody>
	...
	for ( $i = 1; $i <= $max_attributes; $i++ ) {
		...
		if ( !empty( $_POST[ '_attribute-label' . $i ] ) ) {
			...
			if ( $default_label !== $_POST[ '_attribute-label' . $i ] ) {
				$product_meta[ '_attribute-label' . $i ] = apply_filters( 'save_product_attribute_label', $_POST[ '_attribute-label' . $i ], $i, $post->ID );
			} else {
		...
		$product_meta[ '_attribute' . $i ] = isset( $_POST[ '_attribute' . $i ] ) ? apply_filters( 'save_product_attribute_value', $_POST[ '_attribute' . $i ], $i, $post->ID, $label ) : '';
		if ( !empty( $_POST[ '_attribute-unit' . $i ] ) ) {
			$default_unit = get_default_product_attribute_unit( $i );
			if ( $default_unit !== $_POST[ '_attribute-unit' . $i ] ) {

				$product_meta[ '_attribute-unit' . $i ] = apply_filters( 'save_product_attribute_unit', $_POST[ '_attribute-unit' . $i ], $i, $post->ID, $label );
			} else {
