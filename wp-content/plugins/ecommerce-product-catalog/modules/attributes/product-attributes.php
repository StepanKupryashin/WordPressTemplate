	...
	if ( $max_attr > 0 ) {
		...
		if ( in_array( $post->post_status, ic_visible_product_status() ) ) {
			for ( $i = 1; $i <= $max_attr; $i++ ) {
				$attr_ids	 = apply_filters( 'ic_assign_product_attribute', $attr_ids, $product_meta, $i );
				$value_meta	 = '_attribute' . $i;
				$label_meta	 = '_attribute-label' . $i;
				if ( empty( $product_meta[ $value_meta ] ) || (is_array( $product_meta[ $value_meta ] ) && isset( $product_meta[ $value_meta ][ 0 ] ) && empty( $product_meta[ $value_meta ][ 0 ] )) ) {
					continue;
				}
				$attr_ids = array_merge( $attr_ids, ic_assign_product_attribute( $product_meta, $label_meta, $value_meta, $i ) );
			}
		}
